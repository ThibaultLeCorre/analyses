...
Transac_acquereur<- data_redresse%>% 
  filter(`X.x`!= 0 & `Y.x`!=0, !is.na(REQ_ANC))%>%
...
carreaux_resumes <- Cadrillage_200_jointure %>%
  ...
  summarise(Promotion_rapport=(length(which(REQ_ANC=="Logement neuf")) / length(REQ_ANC)) * 100)%>%
  filter(Promotion_rapport>0)

library(ggplot2)
ggplot(carreaux_resumes, aes(x=Promotion_rapport)) + geom_histogram(binwidth = 10)

breaks_ratio<-getBreaks(carreaux_resumes$Promotion_rapport,nclass = 10,method="fisher-jenks")
...
dev.off()
#etape 2 : boucle
spVente$promotion<- ifelse(spVente$REQ_ANC=="Logement neuf",1,0)
for (cetteAnneeLa in annees){
  ...
  stewartPromotion_SF <- left_join(stewartPromotion_SF %>%
                                select(Promotion,Carreau_ID),
                              stewartTransac_df%>%
                                select(NbTransac,Carreau_ID),  
                              by="Carreau_ID") %>%
    mutate(PromotionPotentiel = Promotion/ NbTransac)
