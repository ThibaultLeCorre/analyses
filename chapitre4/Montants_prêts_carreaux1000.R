...
for (cettePeriodeLa in Periode){
  ...
  carreaux_resumes_test <- carreaux_resumes_test %>%
    group_by(Carreau_ID) %>%
    summarise(Credit= sum(MTCRED,na.rm = T),
              Nombre_transacs = n(),
              Prêt_moyen_transac= Credit/Nombre_transacs)
  carreaux_resumes <-carreaux_resumes_test %>% filter(!is.na(Prêt_moyen_transac))
  ...
  stewartBreaks_ratio <- quickStewart(spdf = carreaux_resumes_spdf, spdfid = "Carreau_ID",
                                      df = carreaux_resumes_spdf@data, dfid = "Carreau_ID",
                                      var = "Credit",var2 ="Nombre_transacs",
                                      span = 3000, 
...
for (cettePeriodeLa in Periode){
  ...
  setwd("~/Sauvegarde_figures/Montant_credit_moyen_1000")
  pdf(file=paste0("~/Sauvegarde_figures/Montant_credit_moyen_1000/", cettePeriodeLa,".pdf"),width = 10, height = 5)
  ...
  smoothLayer(spdf = carreaux_resumes_spdf, 
              ...
              mask = Mask_spdf_1000,
              lwd=0.1,
              legend.title.txt = sprintf("Potentiel Montant moyen credit, quantile"),
