...
Transac_acquereur<- data_redresse_avec_promo_2%>% 
  filter(!is.na(CSP_AC))%>%
  select(ID,PRESCREDIT,annee.x,X.x, Y.x, REQTYPBIEN)

Transac_acquereur$Credit <- ifelse(Transac_acquereur$PRESCREDIT=="N", "Sans credit", "Avec credit")
Transac_acquereur$Credit <- ifelse(is.na(Transac_acquereur$PRESCREDIT),"Avec credit",Transac_acquereur$Credit)
unique(Transac_acquereur$Credit)
...
Periode <- unique(spVente$Periode)
...
# breaks_ratio<-getBreaks(carreaux_resumes$taux_credit,nclass = 10,method="fisher-jenks")
...
for (cettePeriodeLa in Periode){
  ...
  carreaux_resumes <- carreaux_resumes_test %>%
    group_by(Carreau_ID) %>%
    summarise(Nombre_Sanscredit=length (which(Credit=="Sans credit")),
              Nombre_transac=length(which(!is.na(Credit))),
              taux_credit = (Nombre_Sanscredit/Nombre_transac)*100)
  carreaux_resumes <-carreaux_resumes %>% filter(!is.na(taux_credit))
  ...
  stewartBreaks_ratio <- quickStewart(spdf = carreaux_resumes_spdf, 
                                      ...
                                      var = "Nombre_Sanscredit",var2 ="Nombre_transac",
                                      span = 3000, 
...
for (cettePeriodeLa in Periode){
  ...
  carreaux_resumes_test <- carreaux_resumes_test %>%
    ...
    summarise(Nombre_Sanscredit=length (which(Credit=="Sans credit")),
              Nombre_transac=length(which(!is.na(Credit))),
              taux_credit = (Nombre_Sanscredit/Nombre_transac)*100)
  carreaux_resumes_test <-carreaux_resumes_test %>% filter(!is.na(taux_credit))
  ...
  setwd("~/Sauvegarde_figures/Achat_sans_credit_carreaux_1000_periode_toutes_transacs")
  pdf(file=paste0("~/Sauvegarde_figures/Achat_sans_credit_carreaux_1000_periode_toutes_transacs/","_", cettePeriodeLa,".pdf"),width = 10, height = 5)
  ...
  smoothLayer(spdf = carreaux_resumes_spdf, 
              ...
              mask = Mask_spdf_1000,
              lwd=0.1,
              legend.title.txt = sprintf("Ratio Potentiel Achat sans credit, quantile"),
