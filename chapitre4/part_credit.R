...
Transac_acquereur<- data_redresse%>% 
  filter(`X.x`!= 0 & `Y.x`!=0, !is.na(PRESCREDIT))%>%
...
carreaux_resumes <- Cadrillage_200_jointure %>%
  group_by(Carreau_ID) %>%
  summarise(Nombre_credit=length (which(Credit=="1")),
            Nombre_transac=length(which(!is.na(PRESCREDIT))),
            taux_credit = length (which(Credit=="1"))/length(which(!is.na(PRESCREDIT)))*100)
...
library(ggplot2)
ggplot(carreaux_resumes, aes(x=taux_credit)) + geom_histogram(binwidth = 1)
...
Montableau_presence_credit<- data.frame(Carreau_ID=NA,NbTransac=NA,taux_credit=NA,Annee=NA)
Montableau_presence_credit<-Montableau_presence_credit%>%filter(!is.na(Annee))
...
for (cetteAnneeLa in annees){
  ...
  stewartCredit_SF <- left_join(stewartCredit_SF %>%
                              ...
                              by="Carreau_ID") %>%
    mutate(taux_credit = (Credit/ NbTransac)*100)
  
  #attention focntionne pas si environnement de travail pas precise
  setwd("~/Sauvegarde_figures/Credit_pourcentage_presence")
  pdf(file=paste0(~"Sauvegarde_figures/Credit_pourcentage_presence",cetteAnneeLa,".pdf"),width = 10, height = 5)
  tilesLayer(osmTiles)
  ...
  Montableau_presence_credit<-bind_rows(Montableau_presence_credit, stewartCredit_df)
...
Montableau_presence_credit_FINAL<- Montableau_presence_credit
