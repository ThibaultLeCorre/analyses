...
table(mytest$Origine_regionale)
#####################
...
Communes_origine_df<-mytest_2%>%
  select(ID,Com_Same_voisine_acaht)
...
Communes_origine_df$origine<-ifelse(Communes_origine_df$Origine_regionale=="Region_IDF"&Communes_origine_df$Com_Same_voisine_acaht=="TRUE" ,"meme_commune_et_voisins","Autre")
Communes_origine_df$origine<-ifelse(Communes_origine_df$origine=="Autre"& Communes_origine_df$Origine_regionale=="Region_IDF"  ,"Autres_communes_de_la_region",Communes_origine_df$origine)
Communes_origine_df$origine<-ifelse( !is.na(Communes_origine_df$PADEPT_AC) & Communes_origine_df$Origine_regionale=="Autres_region", "Autre_region_francaise", Communes_origine_df$origine)
Communes_origine_df$origine<-ifelse(Communes_origine_df$PADEPT_AC==99, "Provenance_etrangere",Communes_origine_df$origine  )

table(Communes_origine_df$origine,Communes_origine_df$BIDEPT, useNA = "ifany")
length(which(Communes_origine_df$BIDEPT==75))
