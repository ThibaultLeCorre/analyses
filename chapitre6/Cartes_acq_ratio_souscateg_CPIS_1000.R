                                 ...
                                 ifelse(mytest$CSP_AC == 34, "Professeurs_universitaires_scientifiques",
                                        ifelse(mytest$CSP_AC == 35, "Informations_arts_spectacles",
                                               ifelse(mytest$CSP_AC == 37, "Cadre_administratif_commercial",
                                                      ifelse(mytest$CSP_AC == 38, "Ingenieur_cadre_technique",NA))))))
###Periode
...
for (cettePeriodeLa in Periodes){
  ...
  carreaux_resumes$Total<- rowSums(as.data.frame(carreaux_resumes[,2:7,drop=TRUE]))
...
opacity <- 85
maPalette <- paste0(carto.pal(pal1 = 'blue.pal', n1=6), opacity)
#etape 2
...
for (cettePeriodeLa in Periodes){
  ...
  for (cetteCSPLa in MesCSP){
    ...
    pdf(file=paste0("~/Projets/Chapitre7/Cartes_acquereurs/Cartes_acq_ratio_souscateg_CPIS/",cetteCSPLa, "_", cettePeriodeLa,".pdf"),width = 10, height = 5)
    tilesLayer(osmTiles)
