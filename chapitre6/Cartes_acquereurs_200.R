...
carreauInsee200_2$Carreau_ID<-1:length(carreauInsee200_2$id_c1000)

cadrillage_spdf<-as(carreauInsee200_2, "Spatial")
...
######################Define breaks
Periodes <- unique(carreauInsee200_jointure$Periode)
...
for (cettePeriodeLa in Periodes){
  ...
  carreaux_resumes$Total<- rowSums(as.data.frame(carreaux_resumes[,2:11,drop=TRUE]))
  ...
  for (cetteCSPLa in MesCSP){
    ...
    stewartBreaks_ratio <- quickStewart(spdf = carreaux_resumes_spdf, 
                                        ...
                                        span = 500, 
                                        beta = 2, mask = Mask.spdf, bypassctrl = TRUE)
...
breaksParCSP_ratio <- allBreaksDF_ratio %>%
  # filter(CSP== "Ouvriers" | CSP== "CPIS" |CSP== "Prof_intermediaires" |CSP== "Employes" ) %>%
  # group_by(CSP) %>%
  gather("Disc", "Value", 2:4) %>%
  summarise(breaksMax = list(quantile(Value, probs = seq(0, 1, 0.2))))


opacity <- 85
cspColors <- list()
cspColors[["Ouvriers"]] <- paste0(carto.pal(pal1 = 'red.pal', n1=5), opacity)
cspColors[["CPIS"]] <- paste0(carto.pal(pal1 = 'blue.pal', n1=5),  opacity)
cspColors[["Prof_intermediaires"]] <- paste0(carto.pal(pal1 = 'green.pal', n1=5),  opacity)
cspColors[["Employes"]] <- paste0(carto.pal(pal1 = 'pink.pal', n1= 5),  opacity)
cspColors[["retraites"]] <- paste0(carto.pal(pal1 = 'grey.pal', n1= 5),  opacity)
cspColors[["Entreprise_Marchands_SCI"]] <- paste0(carto.pal(pal1 = 'sand.pal', n1= 5),  opacity)
cspColors[["Com_art_Chef_entreprises"]] <- paste0(carto.pal(pal1 = 'purple.pal', n1= 5),  opacity)
cspColors[["autres_inactifs"]] <- paste0(carto.pal(pal1 = 'brown.pal', n1= 5),  opacity)
cspColors[["Agriculteurs"]] <- paste0(carto.pal(pal1 = 'grey.pal', n1= 5),  opacity)
cspColors[["Biens_publics_et_HLM"]] <- paste0(carto.pal(pal1 = 'grey.pal', n1= 5),  opacity)
####
...
for (cettePeriodeLa in Periodes){
  ...
  for (cetteCSPLa in MesCSP){
    ...
    pdf(file=paste0("~/Projets/Chapitre7/Cartes_acquereurs/Cartes_acq_ratio_200/",cetteCSPLa, "_", cettePeriodeLa,".pdf"),width = 10, height = 5)
    tilesLayer(osmTiles)
