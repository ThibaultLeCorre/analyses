...
for (cettePeriodeLa in Periodes){
 ...
 # carreaux_filtred <-  carreaux_filtred[sample(1: nrow(carreaux_filtred), 500,replace=T), ]
  ...
  for (cetteCSPLa in MesCSP){
    ...
    pdf(file=paste0("~/Projets/Chapitre7/Cartes_acquereurs/Cartes_acq_NombreByYears_sameScale/",cetteCSPLa, "_", cettePeriodeLa,".pdf"),width = 10, height = 5)
    tilesLayer(osmTiles)
    ...
    smoothLayer(spdf =  carreaux_resumes_spdf, 
                ...
                mask = Mask.spdf,
                lwd=0.1,
                legend.title.txt = sprintf("Nombre Potentiel by year ", cetteCSPLa),
