...
for (cettePeriodeLa in Periodes){
  ...
  MesCSP <- MesCSP[-c(1,2,3,4,5,6,11)]
  ...
  for (cetteCSPLa in MesCSP){
    ...
    # Test_topology<-
      # saveRDS(QuotientLocal_sf,file="~/Projets/Chapitre7/Cartes_acquereurs/QuotientLocal_sf" )
    

    # QuotientLocal_sf<-QuotientLocal_sf%>%
    #   filter(!is.na(QuotientLocal))%>%
    #   st_cast("POLYGON")
    ...
    # 
    # smoothLayer(x=QuotientLocal_sf,
    #             var ="QuotientLocal", var2 = 1,
    #             span = 3000, beta = 2,
    #             col = Mescouleurs,
    #             breaks =Mybreaks,
    #             mask = Mask.st,
    #             lwd=0.1,
    #             legend.title.txt = sprintf("QuotientLocal", cetteCSPLa),
    #             legend.pos = "topright", legend.values.rnd = 3)
    # layoutLayer(title = paste("Nombre par annee", cetteCSPLa, cettePeriodeLa, sep = " - "))
