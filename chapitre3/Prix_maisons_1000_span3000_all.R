...
carreaux_resumes <- Cadrillage_1000_jointure %>%
  ...
  summarise(Nombre_transacs=length(which(REQ_PRIX!="NA")),
            Prix_sum=sum(REQ_PRIX),
            Prix_moyen=Prix_sum/Nombre_transacs)
...
library(ggplot2)
ggplot(carreaux_resumes, aes(x=Prix_moyen)) + geom_histogram(binwidth = 1000)

breaks_ratio<-getBreaks(carreaux_resumes$Prix_moyen,nclass = 10,method="quantile")
########
...
for (cetteAnneeLa in annees){
  ...
  setwd("~/Sauvegarde_figures/Maisons_carreau1000_span3000")
  pdf(file=paste0(~"Sauvegarde_figures/Maison_carreau_1000_span3000",cetteAnneeLa,".pdf"),width = 10, height = 5)
  tilesLayer(osmTiles)
...
Montableau_Maison_1000_all<- Montableau_Maison
rm(carreaux_resumes, breaks_ratio)



Montableau_Maison_1000_all<-Montableau_Maison_1000_all%>% select(-geometry)

setwd("~/Projets/Prix_evol_2/table_results_prix_evol_2")
write.csv2(x=Montableau_Maison_1000_all,file = "Montableau_Maison_1000_all.csv", row.names=FALSE, fileEncoding = "UTF-8")
