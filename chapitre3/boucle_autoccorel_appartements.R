...
titre_diagrammeMoran<- paste0("Diagramme de Moran pour",sep=" ", CeNombredeVoisinsLa, sep=" ","voisins en",sep=" ",cetteAnneeLa) 
 ...
 setwd("~/Projets/Auto_correlation/Autocorellation_spatiale/Diagrammes_Moran/appartements")
 # pdf(file=paste0(~"/Projets/Auto_correlation/Autocorellation_spatiale/Diagrammes_Moran/appartements",titre_diagrammeMoran,".pdf"),width = 10, height = 5)
...
breaks_ratio_resid_moran<-c(min(resid), -sd(resid)*1.5, -sd(resid), -sd(resid)/2, 0, sd(resid)/2, sd(resid), sd(resid)*1.5, max(resid))
# breaks_ratio_resid_moran<-c(min(resid), -sd(resid)*1.5, -sd(resid), -sd(resid)/2, 0, sd(resid)/2, sd(resid), sd(resid)*1.5, max(resid))
...
setwd("~/Projets/Auto_correlation/Autocorellation_spatiale/Cartes_residusecart_moran/appartements")
pdf(file=paste0(~"/Projets/Auto_correlation/Autocorellation_spatiale/Cartes_residusecart_moran/appartements",titre_ResidusMoran,".pdf"),width = 10, height = 5)
