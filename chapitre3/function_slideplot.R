...
GetCrossFlows <- function(df, wgtvar = NULL){
  ...
  df <- plyr::colwise(factor, levels = uniqueVal, labels = uniqueVal)(df)
  ...
  moltenFlows <- reshape2::melt(oriDesTab, id.vars = "WEIGHT", measure.vars = colnames(oriDesTab)[1:(ncol(oriDesTab)-1)])
  ...
  coordFlows <- reshape2::colsplit(flowsTable$FLOWID, pattern = "_", names = c("COORDORI", "COORDDES"))
  coordOri <- reshape2::colsplit(coordFlows$COORDORI, pattern = "-", names = c("XORI", "YORI"))
  coordDes <- reshape2::colsplit(coordFlows$COORDDES, pattern = "-", names = c("XDES", "YDES"))
