...
carreaux_resumes <- Cadrillage_200_jointure %>%
  group_by(Carreau_ID) %>%
  filter(REQTYPBIEN=="MA" & NBRPIECE==4| NBRPIECE==5)%>%
...
for (cetteAnneeLa in annees){
  ...
  spVente_2<-spVente%>% filter(annee.x==cetteAnneeLa, 
                               REQTYPBIEN=="MA" & NBRPIECE==4 | NBRPIECE==5)%>%
    as( "Spatial")
  ...
  setwd("~/Sauvegarde_figures/Maisons_segment4_5pieces_carreau200_span500")
  pdf(file=paste0(~"Sauvegarde_figures/Maisons_segment4_5pieces_carreau200_span500",cetteAnneeLa,".pdf"),width = 10, height = 5)
  tilesLayer(osmTiles)
...
Montableau_Maison_200_segment<- Montableau_Maison
Montableau_Maison_200_segment<-Montableau_Maison_200_segment%>% select(-geometry)
rm(carreaux_resumes, breaks_ratio)

setwd("~/Projets/Prix_evol_2/table_results_prix_evol_2")
write.csv2(x=Montableau_Maison_200_segment,file = "Montableau_Maison_200_segment.csv", row.names=FALSE, fileEncoding = "UTF-8")
